...
To show task names and states, it reads Codex's local task index and lifecycle markers such as task started, task completed, and turn aborted from local rollout logs. For detected command and file-edit approvals, it asks Codex's local log database to classify rows as tool requested, approval resolved, or tool completed; the query returns only that event category and task identifier, never the command or tool body. It parses event metadata and does not copy rollout message or tool content into its own snapshot. The snapshot can contain task identifiers, task titles, working directories, states, and timestamps. A task title may itself have been derived by Codex from the beginning of a user request. Optional Codex lifecycle hooks write only a task identifier, lifecycle event name, and receipt time to small JSON files under `~/Library/Application Support/Signal Monitor/events`. The app stores focused task identifiers, nicknames, layout choices, language, and window position in macOS user preferences.
